...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { name, restaurants } = await getRestaurantsByProgramSlug(slug)

  if (!name || restaurants.length === 0) {
    return {
      title: '프로그램을 찾을 수 없습니다 | 나온집',
      description: '요청하신 프로그램의 부산 맛집 목록을 찾을 수 없습니다.',
    }
  }

  const title = `${name} 부산 맛집 ${restaurants.length}곳 | 나온집`
  const description = `${name}에 소개된 부산 맛집을 나온집에서 모아봤어요.`
  ...
  return (
    <main className="pt-14 pb-24">
      ...
      <section className="px-4 pt-6 pb-5 bg-white">
        <p className="text-xs text-orange-500 font-semibold">방송 출처별 모음</p>
        <h1 className="mt-1 text-2xl font-bold text-gray-900">{name}</h1>
        <p className="mt-2 text-sm text-gray-500">
          부산 맛집 {restaurants.length}곳
        </p>
        <div className="mt-3 space-y-2">
          {getIntro('program', slug, name).paragraphs.map((p, i) => (
