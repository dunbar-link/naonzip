/**
 * 영성방 등록 — 넷플릭스 미친맛집 시즌3 10화 부산편 (사하구 화교 중식).
 *
 * 절차:
 ...
 *   --publish: thumbnail 확정(upsert-restaurant-thumbnails 로 별도 업로드) 확인 후에만 is_published=true.
 * 안전: 비밀키 미출력. 영성방 1건 외 INSERT/UPDATE/DELETE 없음. 좌표/주소/전화 Kakao 검증값(추정 금지).
 * 실행:
 *   node scripts/register-yeongseongbang.mjs            # dry-run(preflight)
 *   node scripts/register-yeongseongbang.mjs --apply    # 비공개 INSERT + appearance
 *   node scripts/register-yeongseongbang.mjs --publish  # 썸네일 확인 후 공개
 */
...
const R = {
  slug: 'michinmatjip-saha-yeongseongbang', name: '영성방', area: '기타',
  address: '부산 사하구 하신번영로201번길 36', category: '중식',
  main_menu: '가지튀김·간짜장·울면',
  price_text: '넷플릭스코스 25,000원 / 간짜장 9,000원 / 울면 11,500원',
  phone: '051-515-7119', source_type: 'tv', source_title: '미친맛집',
  program_name: '미친맛집', episode_title: '시즌3 10화', broadcast_date: null,
  description: '넷플릭스 미친맛집 시즌3에 소개된 부산 사하구 하단의 화교 중식당. 가지튀김·간짜장·울면이 대표.',
}
const PLACE_QUERY = '영성방 사하'
const ADDR_CORE = '하신번영로201번길'
// 미친맛집 출처 trust (미친맛집 3곳 등록과 동일 구조 — 방송 식당 trust 칩)
const TRUST = { source_kind: 'tv', source_name: '미친맛집: 미식가 친구의 맛집', source_title: '시즌3 10화', source_url: 'https://www.busan.com/view/busan/view.php?code=2026020516411355443', trust_label: '넷플릭스 출연', verified_at: '2026-06-25' }
...
async function main() {
  ...
  console.log(`=== 영성방 등록 (${PUBLISH ? 'PUBLISH' : APPLY ? 'APPLY' : 'DRY-RUN/PREFLIGHT'}) ===`)
  ...
  if (PUBLISH) {
    ...
    // trust 연결 (미친맛집, idempotent: restaurant_id + source_name)
  ...
  try {
    ...
    lk = docs.find((x) => { const la = +x.y, ln = +x.x; const a = `${x.road_address_name || ''} ${x.address_name || ''}`; return inBusan(la, ln) && norm(x.place_name).includes(norm('영성방')) && a.includes(ADDR_CORE) })
      || docs.find((x) => inBusan(+x.y, +x.x) && norm(x.place_name).includes(norm('영성방')))
  ...
  const { error: appErr } = await sb.from('restaurant_appearances').insert({
    ...
    broadcast_date: R.broadcast_date, video_url: null, candidate_id: null, note: '미친맛집 시즌3 10화 영성방 — 2026 신규 등록',
  }, { defaultToNull: false })
