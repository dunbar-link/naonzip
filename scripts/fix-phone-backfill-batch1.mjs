/**
 * phone backfill batch 1 — 공개 식당 4곳의 restaurants.phone 만 보강 (DB UPDATE).
 *
 * 출처: quality:audit PHONE_BACKFILL_READY. 안전 기준:
 ...
 *   - phone 외 컬럼(place_id/주소/좌표/상호/slug/category 등) 절대 미수정.
 *   - UPDATE 는 .is('phone', null) 가드로 phone 누락 행만 변경, 영향 행 1 확인.
 *   - 기본 dry-run. 실제 UPDATE 는 --apply 일 때만.
 * 실행:
 *   node scripts/fix-phone-backfill-batch1.mjs          # dry-run
 *   node scripts/fix-phone-backfill-batch1.mjs --apply  # phone UPDATE
 */
...
const TARGETS = [
  { slug: 'saengsaeng-sasang-jurye-suyuk-kalguksu', name: '주례수육칼국수 2호점', expected: '0502-5553-2589' },
  { slug: 'ansungjae-yeonje-mapobonga', name: '마포본가', expected: '051-867-9252' },
  { slug: 'hanyakbang-gukbap-hyeongje-food', name: '한약방돼지국밥 형제식품', expected: '051-646-3102' },
  { slug: 'jeonhyun-gwangalli-biwa-suljan', name: '비와술잔', expected: '051-621-2540' },
]
...
async function main() {
  ...
  console.log(`=== phone backfill batch 1 (${APPLY ? 'APPLY' : 'DRY-RUN'}) — 4곳 phone 만 ===`)
