/**
 * 삼오불고기 등록 — 풍자 '또간집' Ep.01 삼오불고기 (부산 수영구/광안리 대패삼겹살).
 *
 * 절차(register-sundori-boribap 패턴 동일):
 ...
 * 안전: 비밀키 미출력. 삼오불고기 1건 외 INSERT/UPDATE/DELETE 없음. 좌표/주소/전화 Kakao 검증값(추정 금지).
 * 실행:
 *   node scripts/register-samo-bulgogi.mjs            # dry-run(preflight)
 *   node scripts/register-samo-bulgogi.mjs --apply    # 비공개 INSERT + appearance
 *   node scripts/register-samo-bulgogi.mjs --publish  # 썸네일 확인 후 공개 + trust
 */
...
const R = {
  slug: 'ddoganjip-gwangalli-samo-bulgogi', name: '삼오불고기', area: '광안리',
  address: '부산 수영구 수영로610번길 24-2', category: '고기',
  main_menu: '대패삼겹살',
  price_text: '가격 매장 확인',
  phone: '051-754-3610', source_type: 'youtube', source_title: '또간집',
  program_name: '또간집', creator_name: '풍자', episode_title: 'Ep.01 삼오불고기', broadcast_date: null,
  video_url: 'https://www.youtube.com/watch?v=wgw6zRVmja0',
  description: "풍자 '또간집' Ep.01에 소개된 부산 수영구의 대패삼겹살 노포. 얇게 썬 대패삼겹을 불판에 구워 내는 냉삼 스타일이 대표.",
}
const PLACE_QUERY = '삼오불고기 수영구'
const ADDR_CORE = '수영로610번길'
// 또간집(풍자) 유튜브 출처 trust
const TRUST = { source_kind: 'youtube', source_name: '또간집', source_title: 'Ep.01 삼오불고기', source_url: 'https://www.youtube.com/watch?v=wgw6zRVmja0', trust_label: '풍자 또간집 방영', verified_at: '2026-07-24' }
...
async function main() {
  ...
  console.log(`=== 삼오불고기 등록 (${PUBLISH ? 'PUBLISH' : APPLY ? 'APPLY' : 'DRY-RUN/PREFLIGHT'}) ===`)
  ...
  // 사진
  const thumbExists = ['.jpg', '.png', '.webp', '.jpeg'].some((ext) => existsSync(join(THUMB_DIR, `${R.slug}${ext}`)))

  console.log('\n── Kakao preflight ──')
  console.log(`  place_id   : ${placeId ?? '(미확인)'}  (기대: 25039571)`)
  ...
  else if (placeId !== '25039571') { verdict = 'BLOCKED_PLACE_ID_MISMATCH'; reasons.push(`place_id ${placeId}≠25039571`) }
  ...
  if (verdict === 'BLOCKED_DUPLICATE' || verdict === 'BLOCKED_KAKAO_PLACE_UNCONFIRMED' || verdict === 'BLOCKED_PLACE_ID_MISMATCH') { console.error('APPLY 중단: ' + verdict); process.exitCode = 2; return }
  ...
  const { data: ins, error: iErr } = await sb.from('restaurants').insert({
    ...
    thumbnail: null, creator_name: R.creator_name, program_name: R.program_name, episode_title: R.episode_title,
    broadcast_date: R.broadcast_date, description: R.description, video_url: R.video_url,
  ...
  const { error: appErr } = await sb.from('restaurant_appearances').insert({
    ...
    broadcast_date: R.broadcast_date, video_url: R.video_url, candidate_id: null, note: '풍자 또간집 Ep.01 삼오불고기 — 2026 신규 등록(레이더 1회차)',
  }, { defaultToNull: false })
