/**
 * P2 방송/유튜브 trust_source 백필 (preflight 기반).
 *
 * 대상: 공개 restaurants 중 trust 0개 + preflight 분류 P2 (31곳, 전부 READY).
 *   - P2 = READY && 유명출처 키워드 미매칭(P1 아님). source_name(program||creator) 확정 → INSERT.
 *   - REVIEW(program/creator 미상)는 priority P1/P3 으로 분류되어 애초에 P2 아님 → 대상 밖(추정 금지 유지).
 *   - P1 백필분(trust 보유)은 trust 0개 필터에서 제외됨.
 ...
 * 실행:
 *   node scripts/backfill-p2-trust-sources-2026-06.mjs          # dry-run(+report 생성)
 *   node scripts/backfill-p2-trust-sources-2026-06.mjs --apply  # 실제 INSERT
 */
...
// preflight/P1 과 동일한 P1 판정 키워드(반드시 일치 유지 — P2 = READY && !isP1)
...
async function main() {
  ...
  const p2 = targets0.filter((r) => classify(r, hasApp.has(r.id)).priority === 'P2')
  ...
  console.log(`=== P2 trust_source 백필 (${APPLY ? 'APPLY' : 'DRY-RUN'}) ===`)
  console.log(`P2 대상 ${p2.length} / INSERT 예정 ${toInsert.length} / 제외(REVIEW) ${exReview.length} / 제외(no_name) ${exNoName.length} / 중복skip ${skip.length}`)
  ...
  if (APPLY) {
    ...
    writeFileSync(join(outDir, 'p2-trust-source-backfill-dry-run-2026-06.csv'), csv)
    const md = [
      '# P2 trust_source 백필 — DRY-RUN (2026-06)', '',
      `- 스크립트: scripts/backfill-p2-trust-sources-2026-06.mjs (dry-run, DB 수정 0)`,
      `- P2 대상 ${p2.length} / INSERT 예정 ${toInsert.length} / 제외(REVIEW) ${exReview.length} / 제외(no_name) ${exNoName.length} / 중복skip ${skip.length}`,
    ...
    writeFileSync(join(outDir, 'p2-trust-source-backfill-dry-run-2026-06.md'), md)
    console.log(`\nreports/trust-source-backfill/p2-trust-source-backfill-dry-run-2026-06.{csv,md} 생성`)
  }
}
