/**
 * P0 좌표 수정 batch 2 — FIX_READY 3곳의 restaurants.lat/lng 만 교정 (DB UPDATE).
 *
 * 출처: reports/data-quality/p0-address-coordinate-preflight-2026-06 의 FIX_READY_COORDINATE_ONLY 잔여분.
 * 안전:
 *   - lat/lng 외 컬럼(place_id/주소/전화/상호/slug/category/appearance/trust)은 절대 미수정.
 *     ※ 주례수육칼국수 phone backfill 은 이번 batch 에서 금지(좌표만).
 *   - 대상 3 slug 외 UPDATE 금지. 영향 행 1 확인.
 *   - dry-run: DB 현재좌표 + preflight 좌표 + Kakao 주소 재지오코딩 좌표 3중 대조.
 *     preflight ↔ 주소지오코딩 이격 > 200m 이면 해당 식당 BLOCKED(미수정) — 양가네 교훈.
 *   - 기본 dry-run. 실제 UPDATE 는 --apply 일 때만.
 * 실행:
 *   node scripts/fix-p0-coordinates-batch2.mjs          # dry-run
 *   node scripts/fix-p0-coordinates-batch2.mjs --apply  # lat/lng UPDATE
 */
...
// preflight FIX_READY_COORDINATE_ONLY 잔여 3곳 확정 좌표 (CSV kakao_lat/lng)
const TARGETS = [
  { slug: 'jeonhyun-gijang-haenyeo-halmaejib', name: '5번 친구해녀할매집', lat: 35.2204424, lng: 129.2333112 },
  { slug: 'saengbang-gwangalli-sanhae-hoejip', name: '산해횟집', lat: 35.1549965, lng: 129.1316577 },
  { slug: 'saengsaeng-sasang-jurye-suyuk-kalguksu', name: '주례수육칼국수 2호점', lat: 35.1496672, lng: 129.0046757 },
]
...
async function main() {
  ...
  console.log(`=== P0 좌표 수정 batch 2 (${APPLY ? 'APPLY' : 'DRY-RUN'}) — 3곳 lat/lng 만 ===`)
